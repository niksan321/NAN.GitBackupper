...
public class RequestReplay
{
    ...
    /// <summary>
    /// Размеры локальных каталогов бэкапа по ключу DisplayKey (байты), если были посчитаны после синхронизации.
    /// </summary>
    public Dictionary<string, long> RepositoryBackupSizesBytes { get; set; }

    /// <summary>
    /// Сумма размеров выбранных репозиториев (байты); только при полном успехе бэкапа.
    /// </summary>
    public long? LastBackupSelectedTotalBytes { get; set; }

    /// <summary>
    /// Размер созданного ZIP (байты); только при успехе и включённом ZIP.
    /// </summary>
    ...
    public static RequestReplay Error(Exception ex) => new()
    {
        IsSuccess = false,
        IsSended = true,
