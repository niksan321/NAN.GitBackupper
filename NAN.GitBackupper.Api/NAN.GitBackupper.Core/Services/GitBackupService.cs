...
{
    public async Task<RequestReplay> RunBackupAsync(TargetItemModel profile,
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            backupDir = TargetItemBackupPaths.GetEffectiveBackupDirectory(profile);
        ...
        var reposToProcess = repos.Where(r => TargetItemBackupRules.ShouldBackupRepository(profile, r)).ToList();
        ...
        if (profile.CleanBackupDirectory)
        {
            try
            {
        ...
        var degree = Math.Clamp(rawParallelism, 1, processorCount);
        ...
        if (profile.ZipAfterBackup)
        {
            statusProgress?.Report(messages.BackupStepCreatingZip);
            try
            {
                await backupDirectoryArchiveService.CreateArchiveAsync(profile, repositoryFolderNames, ct);
            }
            catch (OperationCanceledException)
            {
                throw;
    ...
    private static string ResolveBackupBranch(TargetItemModel profile, GitRepositoryDescriptor repo)
    {
